...
st_autorefresh(interval=15000 if is_open else 60000, key="clean_sync")

# --- SECTION 2: DATA PERSISTENCE ---
if 'portfolio' not in st.session_state:
    if os.path.exists(PORTFOLIO_FILE):
        df_load = pd.read_csv(PORTFOLIO_FILE)
        # Ensure StopPrice exists for older trades
...
# --- TAB 1: SCANNER ---
with tab1:
    TICKERS = ["SBIN.NS", "BEL.NS", "NTPC.NS", "ASHOKLEY.NS", "ADANIPORTS.NS", "GAIL.NS", "ONGC.NS", "M&M.NS", "HINDALCO.NS", "RELIANCE.NS", "TCS.NS", "INFY.NS", "HDFCBANK.NS", "ICICIBANK.NS"]
    
    @st.cache_data(ttl=30)
    def fetch_market():
        h = yf.download(TICKERS + ["^NSEI"], period="1y", progress=False)
        l = yf.download(TICKERS, period="1d", interval="1m", progress=False)['Close']
        return h, l
...
with tab2:
    ...
    with st.expander("➕ Add Trade"):
        ...
        nt = col1.selectbox("Stock", TICKERS)
